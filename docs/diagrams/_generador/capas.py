...
d = LR("SOMA · capas de código", [
    ("ENTRADA  ·  worker/routes, middleware", GRIS, [
        dict(key='routes', label='routes', sub='Hono, valida la entrada', icon='cloudflare'),
        dict(key='deps',   label='access',         sub='JWT de Access + ADMIN_EMAILS', icon='cloudflare'),
    ]),
    ("CASOS DE USO  ·  worker/services", TEAL, [
        dict(key='svc', label='Servicios de caso de uso', sub='orquestan; no calculan'),
    ]),
    ("NÚCLEO  ·  domain/", NARANJA, [
        dict(key='cost',  label='costing',    sub='AIU, IVA  ·  big.js, nunca float'),
        dict(key='sched', label='simulation', sub='duración y cronograma de obra'),
    ]),
])
...
d.strip("ADAPTADORES  ·  el borde con el mundo — aquí vive TODO el SQL de D1", [
    dict(key='repo', label='repositories/', sub='única puerta a D1', icon='db'),
    dict(key='migr', label='migrations/',   sub='única vía de cambio de esquema', icon='db'),
    dict(key='dbm',  label='db.batch',      sub='escrituras de varios pasos',   icon='db'),
], color='#3A6EA5')
...
d.legend([
    ...
    ("Verificado en tests/architecture/layering.test.ts, no por disciplina", NARANJA),
    ("Un cálculo tiene UNA implementación: la UI importa la misma de domain/", TEAL),
