...
export default function ApuEditor() {
  ...
  async function load() {
    try {
      const [apuData, supplyData] = await Promise.all([
        request("/api/admin/apus"),
        request("/api/admin/supplies"),
      ]);
  ...
  function addSupply() {
    ...
    if (!supply) return;
    setDraft((current) => ({ ...current, detalles: [...current.detalles, { insumo_id: supply.insumo_id, categoria: supply.categoria, nombre_normalizado: supply.nombre_normalizado, unidad_estandar: supply.unidad_estandar, rendimiento: 1, desperdicio_pct: 0, precio_manual: null }] }));
    setNewSupply("");
  }
